...
export function MessageBubble({ data, timestamp }: MessageBubbleProps) {
  ...
  const markdownComponents = useMemo(() => ({
    code({ className, children, ...props }: any) {
      ...
      return isInline ? (
        <code className="bg-muted px-0.5 py-0.5 rounded text-xs font-mono" {...props}>
          {children}
        </code>
      ) : (
    ...
    p({ children }: any) {
      return <p className="mb-1 last:mb-0 text-sm leading-snug">{children}</p>;
    },
    ul({ children }: any) {
      return <ul className="list-disc list-inside mb-1 text-sm">{children}</ul>;
    },
    ol({ children }: any) {
      return <ol className="list-decimal list-inside mb-1 text-sm">{children}</ol>;
    },
    li({ children }: any) {
    ...
    <div className="flex gap-2 group">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        <div className="flex items-center gap-2">
          ...
          {data.usage && (
            <Badge variant="outline" className="text-[8px] h-3 px-1 font-mono ml-auto">
